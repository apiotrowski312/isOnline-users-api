...
func (user *User) Get() rest_errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying prepare get user statmeent", err)
...
func (user *User) Save() rest_errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to get id after creating a new user", saveErr)
		return rest_errors.NewInternalServerError("error when trying to save user", errors.New("database error"))
	}
...
func (user *User) Update() rest_errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to exec update user by id", err)
		return rest_errors.NewInternalServerError("error when trying to update user", errors.New("database error"))
	}
	return nil
}
...
func (user *User) FindByStatus(status string) ([]User, rest_errors.RestErr) {
	...
	if err != nil {
		logger.Error("error when trying to exec find user by status", err)
		return nil, rest_errors.NewInternalServerError("error when trying to find user", errors.New("database error"))
	}
	defer rows.Close()
...
func (user *User) FindByEmailAndPassword() rest_errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying prepare get user by email and password statement", err)
